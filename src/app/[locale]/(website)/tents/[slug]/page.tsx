import { getTentBySlug, getTents } from "@/actions/website";
import { getLocale, getTranslations } from "next-intl/server";
import { notFound } from "next/navigation";
import Image from "next/image";
import Link from "next/link";
import { ArrowLeft, ArrowRight, MessageCircle, Phone } from "lucide-react";
import { TentSizeTable } from "@/components/tents/TentSizeTable";
import { TentSpecificationCard } from "@/components/tents/TentSpecificationCard";
import { TentAccessoriesList } from "@/components/tents/TentAccessoriesList";

const WHATSAPP_NUMBER = '966552248896';
const PHONE_NUMBER = '+966552248896';

interface TentPageProps {
    params: Promise<{
        slug: string;
    }>;
}
...
export default async function TentPage({ params }: TentPageProps) {
    ...
    const t = await getTranslations('TentPage');

    const tent = await getTentBySlug(slug, locale);
    ...
    const whatsappMessage = encodeURIComponent(
        locale === 'ar'
            ...
            : `Hello, I want to inquire about: ${name}`
    );
    ...
    return (
        <div className="min-h-screen bg-background">
            ...
            <div className="relative h-[50vh] min-h-[400px] w-full">
                <Image
                    src={tent.imageUrls[0] || '/assets/placeholder.png'}
                    alt={name}
                    fill
                    sizes="100vw"
            ...
            <div className="container mx-auto px-4 py-12">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        {/* Sizes Table */}
                        {tent.sizes && tent.sizes.length > 0 && (
                            <div className="bg-card rounded-xl p-6 border border-border">
                                <h2 className="text-2xl font-semibold text-foreground mb-6">
                                    {t('sizesTitle')}
                                </h2>
                                <TentSizeTable sizes={tent.sizes} />
                            </div>
                        )}
                        ...
                        {tent.specification && (
                            <div className="bg-card rounded-xl p-6 border border-border">
                                ...
                                <TentSpecificationCard specification={tent.specification} />
                            </div>
                        )}

                        {/* Accessories */}
                        {tent.accessories && tent.accessories.length > 0 && (
                            <div className="bg-card rounded-xl p-6 border border-border">
                                <h2 className="text-2xl font-semibold text-foreground mb-6">
                                    {t('accessoriesTitle')}
                                </h2>
                                <TentAccessoriesList accessories={tent.accessories} />
                            </div>
                        )}

                        {/* Image Gallery */}
                        {tent.imageUrls.length > 1 && (
                            <div className="bg-card rounded-xl p-6 border border-border">
                                <h2 className="text-2xl font-semibold text-foreground mb-6">
                                    {t('gallery')}
                                </h2>
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                    {tent.imageUrls.slice(1).map((url, index) => (
                                        <div
                                            key={index}
